...
describe('PostService', () => {
  ...
  describe('게시글을 id로 조회하는 테스트', () =>{
    it('should get created post before', async () => {
      ...
      const result2 = await service.findById(result.id) as Post;
  ...
  describe('게시물 수정 테스트', () =>{
    it('should change content in post', async () => {
      ...
      const result = await service.updatePost(post1.id, post1) as Post;
    ...
    it('should change content in post with other object', async () =>{
      ...
      const result = await service.updatePost(post.id, post2) as Post;
